    ...
    companion object {
        fun execute(trial: Trial, application: Application, decision: Decision) {
            when {
                ...
                trial.name == "TR_CL_STND" && decision.isNewClient == false && application.persons?.get(0)?.activeScOffer == 1 -> {
                ...
                trial.name == "TR_CC_TW_LG_STND" && decision.isNewClient == false && application.persons?.get(0)?.activeRdOffer == 1 -> {
                ...
                trial.name == "TR_CC_HOMER_POLZA_STND" && decision.isNewClient == false && application.persons?.get(0)?.activeRdOffer == 1 -> {
                    trial.strategyName = "Revolving DM Current"; trial.strategyFlow = "CARD_DM_XSELL"
                }
